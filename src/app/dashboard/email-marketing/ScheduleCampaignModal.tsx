...
import type { AdminEmailRecurrence, SendAdminComposeEmailRequest } from '@/lib/types';
...
export function ScheduleCampaignModal({ open, onClose, compose, estimatedRecipients }: Props) {
  ...
  async function save(status: 'draft' | 'active') {
    ...
    setSaving(status);
    try {
      await apiClient.createAdminEmailSchedule({
        ...
        audienceLabel: estimatedRecipients > 0 ? `Approximately ${estimatedRecipients.toLocaleString()} recipients` : 'Selected campaign audience',
        compose,
      });
      toast.success(status === 'active' ? 'Schedule activated.' : 'Schedule saved as draft.');
      onClose();
    } catch (error) {
  ...
  return (
    <Modal open={open} onClose={onClose} size="xl" labelledBy="schedule-campaign-title" className="max-h-[90vh] overflow-y-auto">
      <div className="flex items-start justify-between border-b border-[var(--color-border-secondary)] p-6">
        <div><p className="mb-1 text-xs font-semibold uppercase tracking-wider text-[var(--color-accent-primary)]">Delivery automation</p><h2 id="schedule-campaign-title" className="text-xl font-bold text-[var(--color-text-primary)]">Schedule this campaign</h2><p className="mt-1 text-sm text-[var(--color-text-secondary)]">Times are stored safely in UTC and continue to follow the selected timezone.</p></div>
      ...
      <div className="flex flex-wrap justify-end gap-3 border-t border-[var(--color-border-secondary)] p-6"><Button type="button" variant="outline" onClick={() => void save('draft')} loading={saving === 'draft'}>Save draft</Button><Button type="button" onClick={() => void save('active')} loading={saving === 'active'} icon={<CalendarClock className="h-4 w-4" />}>Activate schedule</Button></div>
    </Modal>
  );
}
